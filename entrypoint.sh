...
Token_Result=$(curl --insecure --location --request POST ''$INPUT_SERVERURL'/api/auth' \
...
curl --insecure --location --request POST ''${INPUT_SERVERURL}'/api/endpoints/'$INPUT_ENDPOINTID'/docker/images/create?fromImage='$INPUT_IMAGENAME'' \
...
stacks=$(curl --insecure --location --request GET ''${INPUT_SERVERURL}'/api/stacks' \
...
if [ $length -gt 0  ]; then
  ...
  if [[ ! -z $stackId && $stackId -gt 0 ]]; then
    if [ -z "$compose" ]; then
      ...
      file_result=$(curl --insecure --location --request GET ''${INPUT_SERVERURL}'/api/stacks/'${stackId}/file'' \
       --header 'Authorization: Bearer '$token'')
    ...
    #找到同名stack，更新stack
    update_result=$(curl --insecure --location --request PUT ''${INPUT_SERVERURL}'/api/stacks/'${stackId}?endpointId=${INPUT_ENDPOINTID}'' \
     --header 'Authorization: Bearer '$token'' \
...
echo 'create stack  : '${INPUT_SERVERURL}'/api/stacks?endpointId='$INPUT_ENDPOINTID'&method=string&type=1'
if [ -z "$compose" ]; then
...
result=$(curl --insecure --location --request POST ''${INPUT_SERVERURL}'/api/stacks?endpointId='$INPUT_ENDPOINTID'&method=string&type=1' \
--header 'Authorization: Bearer '${token}'' \
--header 'Content-Type: application/json' \
